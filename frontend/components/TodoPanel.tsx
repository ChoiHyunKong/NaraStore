...
const TodoPanel: React.FC<TodoPanelProps> = ({ currentRFPId }) => {
  ...
  return (
    <div className="glass-card rounded-3xl shadow-xl shadow-indigo-100/50 flex flex-col h-full overflow-hidden transition-all border-indigo-50/50">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar px-6 space-y-3 min-h-0">
        {todos.map((todo) => (
      ...
      {!isAdding && (
        <div className="flex-none px-6 pb-4">
          <button
            ...
            className="w-full py-4 border-2 border-dashed border-indigo-100 rounded-2xl text-indigo-400 text-sm font-bold hover:border-indigo-300 hover:bg-indigo-50 hover:text-indigo-600 transition-all flex items-center justify-center gap-2 group"
          >
      ...
      {/* Footer */}
      <div className="flex-none p-6 bg-indigo-50/50 border-t border-indigo-100/50">
